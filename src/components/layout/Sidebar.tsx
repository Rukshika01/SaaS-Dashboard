...
export function Sidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <motion.aside animate={{ width: collapsed ? 84 : 280 }} transition={{ type: 'spring', stiffness: 260, damping: 32 }} className="sticky top-0 hidden h-screen shrink-0 border-r border-[rgb(var(--border))] bg-[rgb(var(--surface))]/72 p-4 backdrop-blur-xl lg:flex lg:flex-col" aria-label="Main sidebar">
      ...
      <nav className="mt-8 space-y-1" aria-label="Primary navigation">{navItems.map((item) => { const Icon = item.icon; const active = page === item.label; return <button key={item.label} onClick={() => { setSelectedProjectId(null); setPage(item.label as Page); }} className={cn('group relative flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition', active ? 'text-[rgb(var(--text))]' : 'text-muted hover:bg-black/5 hover:text-[rgb(var(--text))] dark:hover:bg-white/8')} title={collapsed ? item.label : undefined}>{active && <motion.span layoutId="active-nav" className="absolute inset-y-1 left-1 w-1 rounded-full bg-[rgb(var(--brand))]" />}<Icon size={19} className="shrink-0" /><AnimatePresence>{!collapsed && <motion.span initial={{ opacity: 0, x: -4 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -4 }}>{item.label}</motion.span>}</AnimatePresence></button>; })}</nav>
